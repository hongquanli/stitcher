...
class TestCPUFallback:
    ...
    def test_phase_cross_correlation_cpu(self, force_cpu):
    ...
    def test_shift_array_cpu(self, force_cpu):
    ...
    def test_match_histograms_cpu(self, force_cpu):
        """Test match_histograms with CPU fallback."""
        img = np.random.rand(128, 128).astype(np.float32)
        ref = np.random.rand(128, 128).astype(np.float32) * 2

        result = match_histograms(img, ref)

        assert result.shape == img.shape

    def test_block_reduce_cpu(self, force_cpu):
    ...
    def test_compute_ssim_cpu(self, force_cpu):
        """Test compute_ssim with CPU fallback."""
        arr1 = np.random.rand(128, 128).astype(np.float32)
        arr2 = arr1 + np.random.rand(128, 128).astype(np.float32) * 0.1
...
class TestDtypePreservation:
    ...
    def test_shift_array_dtype(self, dtype, force_cpu):
        """Test shift_array preserves dtype."""
        arr = (np.random.rand(64, 64) * 255).astype(dtype)
        result = shift_array(arr, (1.5, -1.5), preserve_dtype=True)

        assert result.dtype == dtype, f"Expected {dtype}, got {result.dtype}"

    @pytest.mark.parametrize("dtype", [np.uint8, np.uint16, np.float32, np.float64])
    def test_match_histograms_dtype(self, dtype, force_cpu):
        """Test match_histograms preserves dtype."""
        img = (np.random.rand(64, 64) * 255).astype(dtype)
        ref = (np.random.rand(64, 64) * 255).astype(dtype)
        result = match_histograms(img, ref, preserve_dtype=True)

        assert result.dtype == dtype, f"Expected {dtype}, got {result.dtype}"

    @pytest.mark.parametrize("dtype", [np.uint8, np.uint16, np.float32, np.float64])
    def test_block_reduce_dtype(self, dtype, force_cpu):
    ...
    def test_shift_array_no_preserve(self, force_cpu):
...
class TestEdgeCases:
    ...
    def test_shift_zero(self, force_cpu):
    ...
    def test_identical_images_ssim(self, force_cpu):
    ...
    def test_block_reduce_3d(self, force_cpu):
    ...
    def test_different_size_histogram_match(self, force_cpu):
